...
def register_adm_view(request):
    if request.method=="POST":
        ...
        if form.is_valid():     #get data from form (if it is valid)
            ...
            today = date.today()
            age =  today.year - db.year - ((today.month, today.day) < (db.month, db.day))    #calculate age from dob
...
def check_avail(doctor, calldate, calltime):
    try:
        if isinstance(calltime, str):
            calltime = datetime.strptime(calltime, '%I:%M %p').time()  # Convert string time to time object
        return not Appointment.objects.filter(doctor=doctor, calldate=calldate, calltime=calltime).exists()
    except ValueError:
        return False
...
def bookapp_view(request):
    if check_patient(request.user):
        pat = Patient.objects.get(user=request.user)  # Get patient from logged-in user

        # Fetch all pending appointments for the patient
        app_det = [
            [a.doctor.firstname, a.description, a.doctor.department, a.calldate, a.calltime, a.status]
            for a in Appointment.objects.filter(patient=pat, status=False)
        ]
        ...
        if request.method == "POST":
            ...
            if appointmentForm.is_valid():
                ...
                doc = Doctor.objects.filter(id=docid).first()
                calldate = appointmentForm.cleaned_data.get('calldate')
                calltime = appointmentForm.cleaned_data.get('calltime')
                description = appointmentForm.cleaned_data.get('description')

                # Check if doctor is available and date is in the future
                if check_avail(doc, calldate, calltime):
                    if timezone.now().date() < calldate:
                        ...
                        appointmentForm.add_error('calldate', 'Invalid date. Please choose a future date.')
                else:
                    appointmentForm.add_error('calltime', 'Slot Unavailable.')
...
def delete_appointment(request, appointment_id):
    ...
    try:
        ...
        if appointment.patient == patient:
            ...
            recipient_emails = set(
                app.patient.user.email for app in other_appointments
                if app.patient and app.patient.user and app.patient.user.email
            )

            if recipient_emails:
                send_mail(
                    'Appointment Slot Available',
                    f'Hello,\n\nAn appointment with Dr. {appointment.doctor.user.username} on {appointment.calldate} {appointment.calltime} has been canceled. Please check the schedule if you wish to rebook.',
                    'sudhagarlives123@gmail.com',
                    list(recipient_emails),
                    fail_silently=False,
                )
                messages.success(request, "Appointment deleted. Other patients notified.")
            else:
                messages.success(request, "Appointment deleted. No other patients to notify.")
    ...
    return redirect('appoint_view_pat.html')
