...
class AppointmentForm(forms.ModelForm):
    class Meta:
        model = Appointment
        fields = ['description', 'calldate', 'calltime', 'doctor']
        widgets = {
            'description': forms.TextInput(attrs={'class': 'form-control form-control-alternative'}),
            'calldate': forms.DateInput(attrs={'class': 'form-control form-control-alternative'}),
            'calltime': forms.TimeInput(attrs={'class': 'form-control form-control-alternative'}),
            'doctor': forms.Select(attrs={'class': 'form-control form-control-alternative'}),
        }
...
class PatientAppointmentForm(forms.ModelForm):
    doctor = forms.TypedChoiceField(label='')  # doctor is chosen from existing doctors
    doctor.widget.attrs.update({'class': 'app-form-control'})

    calldate = forms.DateField(label='', widget=SelectDateWidget(years=range(2025, 2026)))
    calldate.widget.attrs.update({'class': 'app-form-control-date'})

    calltime = forms.TimeField(label='', widget=forms.Select())
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['doctor'].choices = [(c.id, f"{c.firstname} ({c.department})") for c in Doctor.objects.filter(status=True)]
        
        # Time choices in HH:MM:SS format
        time_choices = [
        ...
        self.fields['calltime'].widget.choices = time_choices
...
class AdminAppointmentForm(forms.ModelForm):     #used to register an appointment by admin
    ...
    calldate = forms.DateField(label='',widget=SelectDateWidget(years=range(2025,2026)))    #date of appointment
    calldate.widget.attrs.update({'class' : 'app-form-control-date'})
